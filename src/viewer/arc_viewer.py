...
class ARCNotebookViewer:
        ...
        for i, example in enumerate(test_examples):
            ...
            if predictions is not None and solution is not None:
                ...
                if i < len(predictions):
                    self._plot_grid_on_axes(predictions[i], ax_pred, f"Test {i + 1} - Prediction")
                else:
    ...
    def _plot_grid_on_axes(self, grid: List[List[int]], ax: plt.Axes, title: str):
        ...
        _ = ax.imshow(grid_array, cmap=self.cmap, norm=self.norm, interpolation="nearest")
    ...
    def batch_analyze(self, n_samples: int = 5, dataset: str = "training"):
        ...
        task_ids = random.sample(list(challenges.keys()), min(n_samples, len(challenges)))
        ...
        for task_id in task_ids:
            ...
            if solution:
                ...
                if hasattr(baseline, "analyze_transformation"):
                    ...
                    print(
                        f"   Pattern detected: {pattern.pattern_type} (confidence: {pattern.confidence:.2f})"
                    )
            print("-" * 60)
    ...
    def compare_predictions(self, baseline, task_id: str, dataset: str = "training") -> None:
        ...
        _ = self.plot_task(task_id, task_data, solution, predictions)
        plt.show()
    ...
    def evaluate_and_visualize(
        ...
        for result in results_to_show:
            ...
            _ = self.plot_task(task_id, task_data, result["solution"], result["predictions"])
            plt.show()
