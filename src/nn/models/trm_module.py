...
class TRMModule(LightningModule):
        ...
        for supervision_step in range(self.hparams.N_supervision):
            ...
            if active_mask.any():
                ...
                loss = F.cross_entropy(
                    ...
                    reduction='none'
                ).view(logits.shape[0], -1).mean(dim=1)  # Per-sequence loss
    ...
    def forward(self, carry: TRMCarry, batch: Dict[str, torch.Tensor]) -> Tuple[TRMCarry, Dict[str, torch.Tensor]]:
        ...
        new_z_H = torch.where(reset_mask, 
                            z_H_init.expand(batch_size, seq_len, -1), 
                            carry.z_H)
        new_z_L = torch.where(reset_mask, 
                            z_L_init.expand(batch_size, seq_len, -1), 
                            carry.z_L)
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int):
        ...
        with torch.no_grad():
            ...
            self.log('val/avg_steps', steps_taken.float().mean(), on_step=False, on_epoch=True, sync_dist=True)
