...
def generate_unique_task_id(concept: str, 
                            filename: str, 
                            used_ids: Set[str],
    ...
    hash_input = f"{concept}_{filename}".encode('utf-8')
    ...
    for attempt in range(max_attempts):
        ...
        hash_input = f"{concept}_{filename}_{attempt}".encode('utf-8')
        hash_hex = hashlib.md5(hash_input).hexdigest()[:8]
        task_id = f"{concept_normalized}_{hash_hex}"
...
def load_tasks_from_directory(corpus_dir: Path) -> Dict[str, Dict]:
    ...
    for concept_dir in sorted(corpus_dir.iterdir()):
        ...
        print(f"  ✓ Loaded {len([t for t in tasks.values() if t['concept'] == concept_name])} tasks from {concept_name}")
...
def split_tasks(tasks: Dict[str, Dict], 
                train_ratio: float = 0.8,
                seed: int = 42) -> Tuple[Dict[str, Dict], Dict[str, Dict]]:
    """
...
def create_arc_format_files(tasks: Dict[str, Dict], 
                            output_dir: Path,
                            prefix: str = "concept_training") -> Tuple[Path, Path]:
    """
...
def print_statistics(tasks: Dict[str, Dict], dataset_name: str):
    ...
    if len(task_ids) != len(unique_ids):
        print(f"⚠️  WARNING: Found duplicate task IDs!")
        print(f"  Total: {len(task_ids)}, Unique: {len(unique_ids)}")
    else:
        print(f"✓ All task IDs are unique")
...
@click.option('--output-dir', type=click.Path(file_okay=False), default='data',
                help='Output directory for ARC-format files')
@click.option('--train-ratio', type=float, default=0.8,
                help='Proportion of tasks for training set')
@click.option('--seed', type=int, default=42,
                help='Random seed for train/eval split')
@click.option('--no-split', is_flag=True,
                help='Do not split into train/eval (put all in training)')
@click.option('--verify-only', is_flag=True,
                help='Only verify uniqueness without creating files')
def main(corpus_dir, output_dir, train_ratio, seed, no_split, verify_only):
    ...
    if eval_tasks:
        ...
        if overlap:
            ...
            return 1
        print(f"✓ No overlap between train and eval sets")
