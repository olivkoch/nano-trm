...
from arc_solver_base import ARCSolver, BaselineSolver, EnsembleSolver, MLSolver, SolverResult
...
class CustomRuleSolver(ARCSolver):
    ...
    def solve_task(self, task_data: Dict) -> SolverResult:
        ...
        for test_case in test_examples:
            ...
            all_3x3 = all(np.array(ex["output"]).shape == (3, 3) for ex in train_examples)
        ...
        return SolverResult(
            predictions=predictions, metadata=metadata, solve_time=time.time() - start_time
        )
...
ensemble = EnsembleSolver(
    solvers=[baseline_solver, custom_solver, neural_solver],
    name="EnsembleVoting",
    strategy="voting",
)
